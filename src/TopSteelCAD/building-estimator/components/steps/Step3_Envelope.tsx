/**
 * Étape 3 : Configuration de l'enveloppe
...
export const Step3_Envelope: React.FC<Step3EnvelopeProps> = ({
  envelopeByStructure,
  extensions,
  buildingDimensions,
  buildingParameters,
  ...
  return (
    <div>
      <div style={{ marginBottom: '30px' }}>
        <h2 style={{ margin: '0 0 10px 0' }}>🏗️ Enveloppe</h2>
        <p style={{ margin: 0, color: '#64748b', fontSize: '0.95rem' }}>
          Configurez le bardage et la couverture pour chaque structure
        </p>
      </div>
        ...
        <div>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              {/* Bardage (selon config) */}
              {typeConfig.components.hasCladding && (
                <div style={formSectionStyle}>
                  <h3 style={{ marginTop: 0 }}>🏗️ Bardage</h3>
                  <CladingEditor
                    structureId={activeStructureId}
                    config={envelopeByStructure[activeStructureId]?.clading}
                    onChange={(config) => onSetClading(activeStructureId, config)}
                  />
                </div>
              )}

              {/* Couverture (selon config) */}
              {typeConfig.components.hasRoofing && (
                <div style={formSectionStyle}>
                  <h3 style={{ marginTop: 0 }}>🏠 Couverture</h3>
                  <RoofingEditor
                    structureId={activeStructureId}
                    config={envelopeByStructure[activeStructureId]?.roofing}
                    onChange={(config) => onSetRoofing(activeStructureId, config)}
                  />
                </div>
              )}
            </div>
          ...
          <div style={{ ...buttonGroupStyleResponsive(isMobile), marginTop: isMobile ? '20px' : '30px' }}>
            ...
            <button style={buttonStyleResponsive('primary', isMobile)} onClick={onNext}>
              {isMobile ? 'Suivant →' : 'Suivant : Finitions →'}
            </button>
          </div>
        </div>
